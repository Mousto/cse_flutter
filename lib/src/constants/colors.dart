import 'package:flutter/material.dart';

/* LISTE DE TOUTES LES COULEURS */
const tPrimaryColor = Color.fromARGB(255, 0, 174, 255);
const tSecondaryColor = Color(0xFF272727);
const tAccentColor = Color.fromARGB(255, 21, 31, 120);
const tSudLinda = Color.fromARGB(255, 156, 39, 176);
const tLindaWelcomeBgColor = Color.fromARGB(255, 242, 234, 243);

const tCardBgColor = Color(0xFFF7F6F1);
const tWhiteColor = Color(0xffffffff);
const tDarkColor = Color(0xFF000000);

/*lES COULEURS D'EMBARQUEMENT À BORD */
const tOnBoardingPage1Color = Colors.white;
const tOnBoardingPage2Color = Color(0xfffddcdf);
const tOnBoardingPage3Color = Color(0xffffdcbd);
